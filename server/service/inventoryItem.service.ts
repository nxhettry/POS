...
export const createInventoryItemService = async (itemData: InventoryItemCreateData) => {
  try {
...
export const updateInventoryItemService = async (id: number, itemData: InventoryItemUpdateData) => {
  try {
    ...
    const updatedItem = await item.update({
      ...itemData,
      lastStockUpdate: itemData.currentStock !== undefined ? new Date() : item.get('lastStockUpdate')
    });
