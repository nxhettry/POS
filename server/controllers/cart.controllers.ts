...
export const getCartsByStatus = asyncHandler(
  async (req: Request, res: Response): Promise<any> => {
    ...
    if (!validStatuses.includes(status)) {
      return res
        .status(400)
        .json(new apiResponse(400, null, "Valid status is required (open, closed, cancelled)"));
    }

    const result = await getCartsByStatusService(status as "open" | "closed" | "cancelled");
...
export const createCartItem = asyncHandler(
  async (req: Request, res: Response): Promise<any> => {
    ...
    if (!cartItemData.cartId || !cartItemData.menuItemId || cartItemData.quantity === undefined) {
      return res
        .status(400)
        .json(new apiResponse(400, null, "Cart ID, menu item ID, and quantity are required"));
    }
